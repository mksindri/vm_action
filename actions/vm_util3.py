...
# Resource Group
GROUP_NAME = 'azure-sample-group-virtual-machines3'

# Network
VNET_NAME = 'azure-sample-vnet3'
SUBNET_NAME = 'azure-sample-subnet3'
...
def get_disc_utilisation(compute_client):
    ...
    k = result.value[0].message
    if k:
        splitted_result = k.split('[stdout]')[1].split('[stderr]')[0]
        output = splitted_result.split('\n')
        output = [o for o in output if o]
        for i in output:
            print(i)

def get_cpu_utilisation(compute_client):
    run_command_parameters = {
        'command_id': 'RunShellScript',  # For linux, don't change it
        'script': [
    ...
    if k:
        splitted_result = k.split('[stdout]')[1].split('[stderr]')[0]
        output = splitted_result.split(',')
        cpu = [k for k in output if 'id' in k]
        cpu_result = cpu[0].replace('id', '')
        print(cpu_result+"%")

def get_top_process(compute_client):
    run_command_parameters = {
        'command_id': 'RunShellScript',  # For linux, don't change it
        'script': [
